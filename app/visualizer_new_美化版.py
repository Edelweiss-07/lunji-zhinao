...
def build_param_cards_html(system_name: str, load_pct: float = 75.0) -> str:
    ...
    for param_name in params:
        ...
        baseline_val = baseline.get("values", {}).get(load_pct, None)
...
def process_chat_and_viz(
    ...
    for idx, chunk in enumerate(gen):
        ...
        # 仅首帧附带图表/状态条/摘要，流式期间只更新对话文本，
        # 避免每个 token 都重发整张 plotly JSON（此前单轮对话流式负载可达数 MB）
        if idx == 0:
            yield (
                history,
                interim_fig,
...
- 控制在合理篇幅内，重点突出。
- **误差表述规范**：参数偏差与容差判定必须使用知识库标注的绝对单位表述（温度用 ±X℃，压力用 ±Xbar，转速用 rpm，油耗用 g/kWh）。严禁把温度、压力、转速的偏差说成"百分之几的误差"；百分比仅允许用于负载率（如 75% 负载）和知识库明确以百分比标注的燃油消耗率区间。判定某参数是否正常时，以其偏差的绝对值是否超过该参数的容差（如排气温度 ±15℃、扫气压力 ±0.2bar、增压器转速差 ≤50rpm）为准。
...
def generate_image_analysis_viz(llm_response: str):
    ...
    valid_units = [unit for _, _, _, _, unit, _ in [rows[i] for i in valid_idx]]
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate=(
            "<b>%{customdata[0]}</b><br>"
            "实测: %{x:.1f}<br>"
            "偏差: %{customdata[1]:+.1f}%{customdata[2]}<br>"
            "<extra></extra>"
        ),
        customdata=list(zip(valid_labels_text, valid_dev, valid_units)),
    ))
